...
const GAME_CONFIG = GAME_CONFIGS["memory-sequence"];

export default function MemorySequenceExperience() {
  ...
  const handleGameOver = useCallback(
    (rounds: number) => {
      if (!qualifies(rounds)) return;
      setPendingScore(rounds);
      setModalOpen(true);
    },
    [qualifies],
  );
    ...
    <>
      <div className="w-full flex flex-col lg:flex-row lg:items-start lg:justify-center gap-6">
        <div className="relative w-full lg:w-[500px] lg:flex-shrink-0">
          <MemorySequenceGame onGameOver={handleGameOver} />
          <button
            type="button"
            onClick={toggleMute}
            aria-label={muted ? "사운드 켜기" : "사운드 끄기"}
            aria-pressed={!muted}
            className="absolute top-2 right-2 z-10 w-9 h-9 rounded-md border border-zinc-700 bg-black/70 backdrop-blur-sm flex items-center justify-center hover:border-fuchsia-400 hover:text-fuchsia-300 text-zinc-300 transition-colors"
...
function HowToPlayCard() {
  const rows: Array<{ label: string; detail: string }> = [
    { label: "WATCH", detail: "시퀀스 점멸을 보고 기억" },
    { label: "YOUR TURN", detail: "본 순서대로 버튼 누르기" },
    { label: "+1 라운드", detail: "맞추면 시퀀스 +1 길어짐" },
    { label: "GAME OVER", detail: "한 번이라도 틀리면 끝" },
    { label: "속도 가속", detail: "10/20 라운드부터 더 빠름" },
  ];
  return (
    <div className="w-full rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
      <h3 className="font-[family-name:var(--font-pixel)] text-[11px] text-fuchsia-400 tracking-widest mb-1">
        ▸ HOW IT WORKS
      </h3>
      ...
      <ul className="space-y-1.5">
        {rows.map((it) => (
          <li key={it.label} className="flex items-baseline gap-3">
            <span className="font-[family-name:var(--font-pixel)] text-[10px] text-fuchsia-300 tracking-wider w-20 shrink-0">
              {it.label}
            </span>
