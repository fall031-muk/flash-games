...
const GAME_CONFIG = GAME_CONFIGS["neon-snake"];

export default function NeonSnakeExperience() {
  ...
  const handleGameOver = useCallback(
    (length: number) => {
      if (!qualifies(length)) return;
      setPendingScore(length);
      setModalOpen(true);
    },
    [qualifies],
  );
    ...
    <>
      <div className="w-full flex flex-col lg:flex-row lg:items-start lg:justify-center gap-6">
        <div className="relative w-full lg:w-[500px] lg:flex-shrink-0">
          <NeonSnakeGame onGameOver={handleGameOver} />
          <button
            type="button"
            onClick={toggleMute}
            aria-label={muted ? "사운드 켜기" : "사운드 끄기"}
            aria-pressed={!muted}
            className="absolute top-2 right-2 z-10 w-9 h-9 rounded-md border border-zinc-700 bg-black/70 backdrop-blur-sm flex items-center justify-center hover:border-lime-400 hover:text-lime-300 text-zinc-300 transition-colors"
...
function HowToPlayCard() {
  const rows: Array<{ label: string; detail: string }> = [
    { label: "← ↑ → ↓", detail: "방향키 / WASD 로 뱀 조종" },
    { label: "스와이프", detail: "모바일: 화면 위에서 방향 스와이프" },
    { label: "반대 방향", detail: "즉시 전환 불가 (자살 방지)" },
    { label: "먹이 = 성장", detail: "먹을수록 길어지고 속도 ↑" },
    { label: "GAME OVER", detail: "벽/자기 몸 충돌 시" },
  ];
  return (
    <div className="w-full rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
      <h3 className="font-[family-name:var(--font-pixel)] text-[11px] text-lime-400 tracking-widest mb-1">
        ▸ HOW IT WORKS
      </h3>
      ...
      <ul className="space-y-1.5">
        {rows.map((it) => (
          <li key={it.label} className="flex items-baseline gap-3">
            <span className="font-[family-name:var(--font-pixel)] text-[10px] text-lime-300 tracking-wider w-20 shrink-0">
              {it.label}
            </span>
