...
import MinesweeperSprintGame from "./MinesweeperSprintGame";
...
const GAME_CONFIG = GAME_CONFIGS["minesweeper-sprint"];

export default function MinesweeperSprintExperience() {
  ...
  const handleGameOver = useCallback(
    (mapsCleared: number) => {
      if (!qualifies(mapsCleared)) return;
      setPendingScore(mapsCleared);
      setModalOpen(true);
    },
    [qualifies],
  );
  ...
  return (
    <>
      <div className="w-full flex flex-col lg:flex-row lg:items-start lg:justify-center gap-6">
        <div className="relative w-full lg:w-[500px] lg:flex-shrink-0">
          <MinesweeperSprintGame onGameOver={handleGameOver} />
          <button
            type="button"
            onClick={toggleMute}
            aria-label={muted ? "사운드 켜기" : "사운드 끄기"}
            aria-pressed={!muted}
            className="absolute top-2 right-2 z-10 w-9 h-9 rounded-md border border-zinc-700 bg-black/70 backdrop-blur-sm flex items-center justify-center hover:border-indigo-400 hover:text-indigo-300 text-zinc-300 transition-colors"
      ...
      <div className="w-full max-w-[500px] mx-auto lg:hidden mt-6">
        <Leaderboard
          game={GAME_CONFIG.key}
          entries={currentPeriodData.entries}
          configured={board.configured}
...
function HowToPlayCard() {
  const rows: Array<{ label: string; detail: string }> = [
    { label: "탭 / 클릭", detail: "셀 공개 (첫 클릭 안전)" },
    { label: "우클릭 / F", detail: "깃발 모드 토글 (모바일도)" },
    { label: "숫자", detail: "주변 8칸 지뢰 개수" },
    { label: "맵 클리어", detail: "+1 맵, 새 맵 자동 생성" },
    { label: "지뢰 클릭", detail: "−3초 페널티 + 새 맵" },
  ];
  return (
    <div className="w-full rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
      <h3 className="font-[family-name:var(--font-pixel)] text-[11px] text-indigo-400 tracking-widest mb-1">
        ▸ HOW IT WORKS
      </h3>
      ...
      <ul className="space-y-1.5">
        {rows.map((it) => (
          <li key={it.label} className="flex items-baseline gap-3">
            <span className="font-[family-name:var(--font-pixel)] text-[10px] text-indigo-300 tracking-wider w-20 shrink-0">
              {it.label}
            </span>
